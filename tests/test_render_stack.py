...
def test_resolve_render_plan_interprets_structured_phrase_window_labels(tmp_path: Path):
    p1 = tmp_path / 'a.wav'
    p2 = tmp_path / 'b.wav'
    sf.write(p1, np.zeros(44100 * 12, dtype=np.float32), 44100)
    sf.write(p2, np.zeros(44100 * 12, dtype=np.float32), 44100)
    ...
    b = make_song(str(p2), 120.0, 'C', 'major', '8B', 1, 0.1)
    a.structure['phrase_boundaries_seconds'] = [0.0, 4.0, 8.0, 12.0]
    plan = _single_section_plan(source_parent='A', bar_count=4, source_section_label='opening_intro_0_3')

    manifest = resolve_render_plan(plan, a, b)

    source = manifest.sections[0].source
    assert source.raw_start_sec == 0.0
    assert source.raw_end_sec == 12.0
    assert source.snapped_start_sec == 0.0
    assert source.snapped_end_sec == 12.0
    joined = '\n'.join(manifest.warnings + manifest.fallbacks + manifest.sections[0].warnings)
...
def test_resolve_render_plan_handoff_support_profile_prefers_explicit_planner_risk_signal(tmp_path: Path):
    p1 = write_sine(tmp_path / 'a.wav', 220.0)
    p2 = write_sine(tmp_path / 'b.wav', 660.0)
    a = make_song(str(p1), 120.0, 'A', 'minor', '8A', 1, 0.1)
    b = make_song(str(p2), 120.0, 'C', 'major', '8B', 1, 0.1)
    ...
    high_risk_plan = ChildArrangementPlan(
        ...
        compatibility=CompatibilityFactors(tempo=1.0, harmony=1.0, structure=1.0, energy=1.0, stem_conflict=1.0),
        sections=[replace(base_section, support_transition_risk=0.8)],
    )
    low_risk_plan = ChildArrangementPlan(
        parents=high_risk_plan.parents,
        compatibility=high_risk_plan.compatibility,
        sections=[replace(base_section, support_transition_risk=0.28)],
    )
...
def test_resolve_render_plan_handoff_support_profile_tightens_low_viability_payoff_handoffs(tmp_path: Path):
    ...
    base_section = PlannedSection(
        ...
        transition_mode='arrival_handoff',
        support_gain_db=-10.0,
        support_transition_risk=0.58,
        support_foreground_collision_risk=0.5,
    )
    low_viability_plan = ChildArrangementPlan(
        parents=[
            ParentReference(str(p1), 120.0, 'A', 'minor', 12.0),
            ParentReference(str(p2), 120.0, 'C', 'major', 12.0),
        ],
        compatibility=CompatibilityFactors(tempo=1.0, harmony=1.0, structure=1.0, energy=1.0, stem_conflict=1.0),
        sections=[replace(base_section, support_transition_viability=0.3)],
    )
    high_viability_plan = ChildArrangementPlan(
        parents=low_viability_plan.parents,
        compatibility=low_viability_plan.compatibility,
        sections=[replace(base_section, support_transition_viability=0.75)],
    )
...
def test_resolve_render_plan_normalizes_transition_mode_hyphenated_variants(tmp_path: Path):
    p1 = tmp_path / 'a.wav'
    p2 = tmp_path / 'b.wav'
    sf.write(p1, np.zeros(44100 * 12, dtype=np.float32), 44100)
    sf.write(p2, np.zeros(44100 * 12, dtype=np.float32), 44100)
    ...
    plan = ChildArrangementPlan(
        ...
        sections=[
            ...
            PlannedSection(label='payoff', start_bar=4, bar_count=4, source_parent='B', source_section_label='phrase_0_2', transition_in='blend', transition_mode='  Single-Owner Handoff  '),
        ],
    )
...
def test_resolve_render_plan_caps_same_parent_flow_blend_to_payoff_at_half_beat(tmp_path: Path):
    p1 = tmp_path / 'a.wav'
    p2 = tmp_path / 'b.wav'
    sf.write(p1, np.zeros(44100 * 12, dtype=np.float32), 44100)
    sf.write(p2, np.zeros(44100 * 12, dtype=np.float32), 44100)
    ...
    plan = ChildArrangementPlan(
        ...
        sections=[
            PlannedSection(label='verse', start_bar=0, bar_count=4, source_parent='A', source_section_label='phrase_0_2'),
            PlannedSection(label='payoff', start_bar=4, bar_count=4, source_parent='A', source_section_label='phrase_0_2', transition_in='blend', transition_mode='same_parent_flow'),
        ],
    )
    ...
    assert manifest.sections[1].overlap_beats_max == 0.5
    joined = '\n'.join(manifest.warnings + manifest.fallbacks + manifest.sections[1].warnings)
    assert 'transition_mode=same_parent_flow capped overlap from 8.0 to 0.5 beats' in joined
...
def test_apply_support_entry_shape_build_tail_notches_vocal_presence_mids_more_than_verse_tail():
    class Dummy:
        def __init__(self, **kwargs):
            self.__dict__.update(kwargs)
    ...
    work = Dummy(order_type='section_support', role='filtered_support', fade_in_sec=0.25, fade_out_sec=0.7, target_duration_sec=seconds)
...
def test_apply_support_entry_shape_build_entry_notches_high_crowding_handoff_more_than_low_crowding_handoff():
    class Dummy:
        def __init__(self, **kwargs):
            self.__dict__.update(kwargs)
    ...
    high = _apply_support_entry_shape(segment, sr, high_crowding, Dummy(label='build', transition_mode='arrival_handoff'))
    low = _apply_support_entry_shape(segment, sr, low_crowding, Dummy(label='build', transition_mode='arrival_handoff'))
...
def test_apply_support_entry_shape_build_tail_notches_high_crowding_handoff_more_than_low_crowding_handoff():
    class Dummy:
        def __init__(self, **kwargs):
            self.__dict__.update(kwargs)
    ...
    high = _apply_support_entry_shape(segment, sr, high_crowding, Dummy(label='payoff', transition_mode='single_owner_handoff'))
    low = _apply_support_entry_shape(segment, sr, low_crowding, Dummy(label='payoff', transition_mode='single_owner_handoff'))

    tail_slice = slice(int(1.78 * sr), int(1.98 * sr))
    t_tail = t[: tail_slice.stop - tail_slice.start]
    carrier = np.sin(2 * np.pi * 1000.0 * t_tail)
    high_proj = np.abs(np.mean(high[0, tail_slice] * carrier))
    low_proj = np.abs(np.mean(low[0, tail_slice] * carrier))
