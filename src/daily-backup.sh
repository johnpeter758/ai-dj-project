...
for file in "${BACKUP_FILES[@]}"; do
    if [ -f "$file" ]; then
        ...
        # Scan for secrets and replace
        cp "$file" "$dest"
        
        # Replace known secret patterns
        sed -i '' -E 's/(token["\']?[:=]["\']?)[a-zA-Z0-9_\-]{20,}/\1[REDACTED]/g' "$dest"
        sed -i '' -E 's/(password["\']?[:=]["\']?)[^"'"'"'\s]{4,}/\1[REDACTED]/g' "$dest"
        sed -i '' -E 's/(api[_-]?key["\']?[:=]["\']?)[a-zA-Z0-9_\-]{10,}/\1[API_KEY]/g' "$dest"
        sed -i '' -E 's/(DISCORD_BOT_TOKEN)[=][^"'"'"'\s]+/\1=[DISCORD_BOT_TOKEN]/g' "$dest"
        sed -i '' -E 's/(Authorization:)[^"'"'"'\n]*/\1 [REDACTED]/g' "$dest"
